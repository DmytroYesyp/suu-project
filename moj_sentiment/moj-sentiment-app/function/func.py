...
class Function:
    ...
    async def handle(self, scope, receive, send):
        logging.info("CloudEvent function invoked")

        # receive the entire HTTP request body (CloudEvent JSON)
        event_body = b""
        more_body = True
        while more_body:
            message = await receive()
            event_body += message.get("body", b"")
            more_body = message.get("more_body", False)

        try:
            event = json.loads(event_body.decode())
            logging.info(f"Received CloudEvent: {event}")
        except Exception as e:
            logging.error(f"Failed to decode CloudEvent: {e}")
            # Respond with 400 Bad Request if decoding fails
            await send({
            ...
            await send({
                ...
                "body": b"Invalid CloudEvent JSON",
            })
            return

        # Extract your custom data from the event
        data = event.get("data", {})
        review_text = data.get("reviewText", "")

        # Your logic here: e.g. sentiment analysis
        result = "neutral"
        import random
        if random.random() <= 0.5:
            result = "negative"

        # Construct a simple response CloudEvent (optional)
        response_event = {
            "specversion": "1.0",
            "type": "com.example.sentiment.response",
            "source": "/moj-sentiment-app",
            "id": event.get("id", "response-id"),
            "data": {
                "sentiment": result,
        ...
        response_body = json.dumps(response_event).encode()
